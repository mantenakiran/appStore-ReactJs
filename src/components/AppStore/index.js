...
class AppStore extends Component {
  state = {searchInput: '', appLists: tabsList[0].tabId}
  ...
  filterSearchResult = () => {
    ...
    const searchResult = appsList.filter(eachItem =>
  ...
  listCategoryItems = tabId => {
    this.setState({appLists: tabId})
  }

  getFilteredList = searchApps => {
    const {appLists} = this.state

    const filteredResult = searchApps.filter(
      eachItem => eachItem.category === appLists,
    )
  ...
  render() {
    const {searchInput, appLists} = this.state
    const getSearchResult = this.filterSearchResult()
    const getFilteredList = this.getFilteredList(getSearchResult)
      ...
      <div className="bg-container">
        ...
        <ul className="tabs-container">
          {tabsList.map(eachItem => (
            <TabItem
              isTabClicked={this.listCategoryItems}
              key={eachItem.tabId}
              tabDetails={eachItem}
              isActive={appLists === eachItem.tabId}
            />
          ))}
        </ul>

        <ul className="app-list-container">
          {getFilteredList.map(eachItem => (
